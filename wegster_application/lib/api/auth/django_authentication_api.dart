// ignore_for_file: unused_local_variable, unnecessary_brace_in_string_interps, avoid_print, non_constant_identifier_names, implementation_imports, unused_import
...
Future<dynamic> posthotelKhaltibooking(
  String Room_Type,
  String Bed_Type,
  String price_per_night,
  String Refundable_Option,
...
Future<dynamic> postbusbooking(
  ...
  String Payment = "cash_on_Arrival";
  String Order_Status = "Pending";
  bool Payment_Complete = false;
  var res = await http.post(url,
      headers: <String, String>{
...
Future<dynamic> postKhaltibusbooking(
    String name,
    String source,
    String dest,
    int user,
...
Future<List<Busbooking>> GetUserBusBooking(int id) async {
  ...
  String FK = id.toString();
  const String api = ("$baseUrl/user/BusBookUser/");
  var res = await http.get(Uri.parse(api + FK));
  print(res.statusCode);
...
Future<List<Busbooking>> GetBusadmin() async {
  List<Busbooking> myTodos = [];
  const String api = '$baseUrl/user/BusBook/';
  var res = await http.get(Uri.parse(api));
  print(res.statusCode);
...
class FetchHotelLocation {
  var data = [];
  List<Hotel> results = [];
  String api = '$baseUrl/user/Hotel/';
  Future<List<Hotel>> getHotelLocation({String? query}) async {
    var url = Uri.parse(api);
    var res = await http.get(url);
    try {
      if (res.statusCode == 200) {
        ...
        if (query != null) {
          results = results
              .where((element) => element.Hotel_Location!
                  .toLowerCase()
                  .contains(query.toLowerCase()))
              .toList();
        }
